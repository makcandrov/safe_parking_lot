...
A generic, lightweight wrapper around different types of locks that leverages Rust’s borrow checker to guarantee correct conditional write patterns.

When writing code that may or may not modify the contents of a lock depending on runtime conditions, it is easy to accidentally:
...
All of these can lead to subtle logic bugs that the compiler cannot detect when using a raw lock.

`SafeLock` prevents these patterns entirely by using the Rust type system.
